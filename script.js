...
function initChat() {
    ...
    if (startChatBtn) {
        startChatBtn.addEventListener('click', function() {
            ...
            // 查找对话页面的链接并模拟点击
            const chatLink = document.querySelector('a[href="#chat-section"]');
            if (chatLink) {
                chatLink.click();
            } else {
                // 如果找不到链接，直接调用导航函数
                ...
                const navLinks = document.querySelectorAll('#sidebar a');
                navLinks.forEach(link => {
                    link.classList.remove('active', 'bg-blue-50');
                    if (link.getAttribute('href') === '#chat-section') {
                        link.classList.add('active', 'bg-blue-50');
                    }
                });
            }
        });
...
// 发送聊天消息
async function sendChatMessage(message, containerId) {
    ...
    thinkingDiv.className = 'max-w-[80%] flex flex-col self-start';
    
    const thinkingContent = document.createElement('div');
    thinkingContent.className = 'p-4 rounded-3xl bg-blue-500 text-white relative break-words';
    thinkingContent.style.animation = 'messageAppear 0.3s ease-out';
    
    // 使用思考状态文本
    const thinkingResponse = wallEResponses.thinking[Math.floor(Math.random() * wallEResponses.thinking.length)];
    thinkingContent.textContent = thinkingResponse;
    ...
    try {
        // 检查智谱AI是否已加载
        if (window.zhipuAI && typeof window.zhipuAI.callAI === 'function') {
            console.log('使用智谱AI处理请求...');
            
            // 调用智谱API获取回复
            const aiResponse = await window.zhipuAI.callAI(message);
            
            // 为回复添加智谱AI标识
            const smartResponse = `${aiResponse} [智谱AI]`;
            
            // 替换思考中的消息为AI回复
            thinkingContent.textContent = smartResponse;
        } else {
            console.log('智谱AI未加载，使用本地回复...');
            ...
            setTimeout(() => {
                ...
                if (Math.random() < 0.7) {
                    // 尝试根据关键词匹配回复
                    botResponse = generateWallEResponse(message);
                } else {
                    // 30%几率使用更具体的随机回复
                    const botResponses = [
                        "*beep* 我正在尽力理解您的问题... *whirr* 我的功能还在开发中。",
                        "这个问题很有趣！*click* 未来我会通过智谱AI来提供更好的回答。",
                        "*whirr* 正在处理... 目前我只能提供简单回复，智能回答即将推出！",
                        "有趣的提问！*beep* 我的智能还在升级中，期待智谱AI接入后的表现！",
                        "*click-click* 我注意到智谱AI未连接。暂时只能以基础模式回应。",
                        "*beep* 智谱AI接口未检测到。*whirr* 我正在以本地模式回答。",
                        "让我思考一下... *processing* 这个问题需要更强大的AI来回答。"
                    ];
                ...
                // 更新思考中的消息为回复内容
                thinkingContent.textContent = botResponse;
            }, delay);
        }
    } catch (error) {
        console.error('AI响应处理错误:', error);
        // 显示错误消息
        thinkingContent.textContent = "*buzz* 抱歉，我现在无法正常回应。*whirr* 请稍后再试。 [错误]";
    }
}
